...
pntsArray = []
dynamic = open("./BlockSize/07/dynamic.out", "r")
...
static = open("./BlockSize/07/sparse.out", "r")
staticArray = list(map(float, static.read().split(",")))


for subdir, dirs, files in os.walk('BlockSize'):
    ans = []
...
plt.plot(y, z)
plt.xlabel('Parameter (Y where YxY is Resolution)')
