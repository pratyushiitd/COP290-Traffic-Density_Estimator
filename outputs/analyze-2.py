...
pntsArray = []
origDynamic = list(map(float, open("./method_2/1920x1080/dynamic.out").read().split(",")))
origQueue = list(map(float, open("./method_2/1920x1080/static.out").read().split(",")))
d = []
q = []
t = []
x = []
for subdir, dirs, files in os.walk('method_2'):
    ans = []
    for file in files:
        print(file)
        if file == 'dynamic.out':
            ...
            print(subdir)
            print(subdir.index('/'))
            ans += [int(subdir[subdir.index('/')+1:subdir.index('x')]), x]
            # x = float(open(os.path.join(subdir, file)).read())
...
#plt.show()
plt.savefig("./method_2/dynamic error")
plt.close()
plt.plot(y, x)
plt.xlabel("Resolution")
plt.ylabel('% Error in Queue Density')
#plt.show()
plt.savefig("./method_2/queue error")
plt.close()
plt.plot(y, z)
plt.xlabel("Resolution")
plt.ylabel('Execution time (sec)')
#plt.show()
plt.savefig("./method_2/time_taken")
plt.close()