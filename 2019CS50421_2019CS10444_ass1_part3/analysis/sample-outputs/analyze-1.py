...
pntsArray = []
origDynamic = list(map(float, open("./method_1/4/dynamic.out").read().split(",")))
origQueue = list(map(float, open("./method_1/4/static.out").read().split(",")))
d = []
q = []
t = []
x = []
for subdir, dirs, files in os.walk('method_1'):
    ans = []
    for file in files:
        print(file)
        if file == 'dynamic.out':
...
#plt.show()
plt.savefig("./method_1/dynamic error")
plt.close()
plt.plot(y, x)
plt.xlabel("Parameter X where N and N+X are proccesed")
plt.ylabel('% Error in Queue Density')
#plt.show()
plt.savefig("./method_1/queue error")
plt.close()
plt.plot(y, z)
plt.xlabel("Parameter X where N and N+X are proccesed")
plt.ylabel('Execution time (sec)')
#plt.show()
plt.savefig("./method_1/time_taken")
plt.close()